    ...
    function create_post()
    {
        ...
        if(empty($input['id'])) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $isCreate = $this->post->createPost($record,null,$account);
                if ($isCreate) {
                    $status = 'success';
                    $message = 'insert post successfully!';
                    //push notify for province when user create post
                    $this->notify->send_notify_to_providers($isCreate);
                }
            }
        }
        /*update post ------------------------------------------------------------------*/
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $isUpdate = $this->post->createPost($record,$input['id'],$account);
                if ($isUpdate) {
                    $status = 'success';
    ...
    function emergency_post(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($isSavePost) {
                ...
                $message = '';
                //push notify for province when user create post
                $this->notify->send_notify_to_providers($isSavePost);
            }
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($account['id'] == $post_info->created_by || $account['id'] == $post_info->picked_by){
                ...
                if($destroyStatus){
                    ...
                    try {
                        $this->notify->send_notify_account(array($input['id']),1,3,$account['id'],$post_info->picked_by);
                    }catch (ErrorException $e){
                        //return null;
                    }
                }else{
