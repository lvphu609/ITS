...
// check account exits in database by username and password
    ...
    //check existed cardId in database
    function checkCardIdUniqueToUpdateAccount($account_id,$cardId){
        $this->db->select('*');
        $this->db->from('accounts');
        $this->db->where(array(
    ...
    function getAccountIdByLocation($post, $RADIUS = API_RADIUS_NUMBER){
